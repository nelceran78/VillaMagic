...
namespace MagicVilla_Web.Controllers
{
    public class NumeroVillaController : Controller
    {
        ...
        public async Task<IActionResult> IndexNumeroVilla()
        {
            ...
            {
                numeroVillaList = JsonConvert.DeserializeObject<List<NumeroVillaDto>>
        ...
        public async Task<IActionResult> CrearNumeroVilla()
        {
            ...
            if(response !=null && response.IsExitoso)
            {
                numeroVillaVM.VillaList = JsonConvert.DeserializeObject<List<VillaDto>>
                                (Convert.ToString(response.Resultado)).Select(v => new SelectListItem
                                { 
                                    Text = v.Nombre,
                                    Value = v.Id.ToString() 
                                });
        ...
        public async Task<IActionResult> CrearNumeroVilla(NumeroVillaViewModel modelo)
        {
            ...
            {
                modelo.VillaList = JsonConvert.DeserializeObject<List<VillaDto>>
                                (Convert.ToString(res.Resultado)).Select(v => new SelectListItem
                                {
                                    Text = v.Nombre,
                                    Value = v.Id.ToString()
                                });
